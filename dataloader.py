...
class GenericDataset(data.Dataset):
    ...
    def _keep_first_k_examples_per_category(self, num_imgs_per_cat):
        print(f'num_imgs_per_category {num_imgs_per_cat}')

        if self.dataset_name == 'cifar10':
            pretrain_labels = self.data.test_labels if (
                self.split == 'test') else self.data.pretrain_labels
            pretrain_data = self.data.test_data if (
                self.split == 'test') else self.data.pretrain_data
            ...
            pretrain_data = data[all_indices]
            pretrain_labels = [labels[idx] for idx in all_indices]
            if self.split == 'test':
...
if __name__ == '__main__':
    ...
    dataloader = DataLoader(
        dataset, batch_size=configs.batch_size, unsupervised=True)
